<template>
  <div class="fixed z-10 flex flex-col items-center w-auto p-2 bg-white rounded dark:bg-black top-2 left-4 bg-opacity-10 dark:bg-opacity-60 backdrop-blur lg:flex-row">
    <nav class="flex flex-col font-bold lg:flex-row">
      <a v-for="(text, cate) in categoryNav" 
         :key="cate"
         class="px-2 cursor-pointer group"
         @click="loadPage(cate)">
        <span class="relative">
          {{ text }}
          <span class="absolute bottom-0 block w-0 h-1 transition-all duration-500 border-b-2 border-black group-hover:w-full dark:border-white"
                :class="cate === categorySelected ? 'w-full' : ''">
          </span>
        </span>
      </a>
    </nav>
...
const categoryNav: Partial<Record<CategoryKey, string>> = {
  article: 'Article',
  furni: 'Mobilier',
  badge: 'Badge',
  catalogue: 'Catalogue',
...
const emit = defineEmits(['openSearch', 'loadPage'])
...
const loadPage = (category: CategoryKey) => {
  emit('loadPage', category)
}
